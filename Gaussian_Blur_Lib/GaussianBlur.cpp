...
void blur_image(BYTE image[], BYTE result_image[], BYTE mask[], int mask_sum, int width, int height)
{
	...
	int padded_width_bytes = (width + 2) * 4;
	...
	for (int i = padded_width_bytes, j = 0; i < image_size_bytes - padded_width_bytes; i++)
	{
		...
		if (i % padded_width_bytes == 0 || i % padded_width_bytes == (padded_width_bytes - 4))
		{
			i += 3;
			continue;
		}
		...
		color_sum += image[i - padded_width_bytes - 4] * mask[0]; // Top left pixel
		color_sum += image[i - padded_width_bytes] * mask[1]; // Top middle pixel
		color_sum += image[i - padded_width_bytes + 4] * mask[2]; // Top right pixel

		color_sum += image[i - 4] * mask[3]; // Middle left pixel
		color_sum += image[i] * mask[4]; // Center pixel
		color_sum += image[i + 4] * mask[5]; // Middle right pixel

		color_sum += image[i + padded_width_bytes - 4] * mask[6]; // Bottom left pixel
		color_sum += image[i + padded_width_bytes] * mask[7]; // Bottom middle pixel
		color_sum += image[i + padded_width_bytes + 4] * mask[8]; // Bottom right pixel
